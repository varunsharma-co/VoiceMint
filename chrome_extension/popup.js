document.addEventListener("DOMContentLoaded", () => {
  ...
  const formattedOutputContainer = document.getElementById(
  ...
  // --- LLM Display Name Dictionary ---
  // Add new LLMs here. The key must perfectly match the string sent by the backend.
  ...
  async function saveState() {
    ...
    const state = {
      ...
      isFormattedVisible:
        !formattedOutputContainer.classList.contains("hidden"),
  ...
  async function restoreState(tabId) {
    ...
    if (state) {
      ...
      if (state.selectedProvider)
        llmProviderSelect.value = state.selectedProvider;
    }
    ...
    chrome.scripting.executeScript(
      ...
      (results) => {
        if (results && results[0] && results[0].result) {
          ...
          if (
            selectedText.trim() &&
            selectedText.trim() !== transcriptOutput.value.trim()
          ) {
      ...
      try {
        ...
        if (data.action === "init_response") {
          ...
          providers.forEach((prov) => {
            ...
            // Look up the name in our dictionary first
            let displayName = PROVIDER_DISPLAY_NAMES[prov];

            // Fallback to auto-formatting if it's not in the dictionary
            if (!displayName) {
              displayName = prov
                .replace(/_/g, " ")
                .split(" ")
  ...
  async function initialize() {
    ...
    copyRawBtn.addEventListener("click", () =>
      handleCopy(transcriptOutput, copyRawBtn),
    );
    copyFormattedBtn.addEventListener("click", () =>
      handleCopy(formattedOutput, copyFormattedBtn),
    );
  }
