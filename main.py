...
import history
import utils
from text_injection import close_injector, get_injector, VirtualKeyboard
import ui
import stt
import llm

def queue_consumer(vkb: VirtualKeyboard) -> None:
    """
    A persistent daemon thread that watches the transcript_queue.
    Checks utils.app_running to ensure a graceful exit.
    """
    injector = get_injector(vkb)
    session_text = ""
...
if __name__ == "__main__":
    ...
    with VirtualKeyboard() as vkb:
        ...
        # Consumer thread
        consumer_thread = threading.Thread(target=queue_consumer, args=(vkb,), daemon=False)
        ...
        # 3. Clean up injector logic (Hardware is handled by vkb Context Manager)
        close_injector()
